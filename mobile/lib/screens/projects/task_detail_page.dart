...
class _TaskDetailPageState extends State<TaskDetailPage> {
  ...
  void _showLockedSnack() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('task_detail.evidence.locked_snack'.tr())),
    );
  }
  ...
  Future<void> _uploadFiles(List<MultipartFile> files) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('task_detail.evidence.uploaded'.tr())),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'task_detail.evidence.upload_failed_with_error'
  ...
  Future<void> _deleteOne(int id) async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('task_detail.evidence.deleted_one'.tr())),
    );
  }
  ...
  Future<void> _clearAll() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('task_detail.evidence.deleted_all'.tr())),
    );
  }
        ...
        child: Wrap(
          children: [
            ListTile(
              ...
              title: Text('task_detail.evidence.take_photo'.tr()),
              onTap: () => Navigator.pop(context, 'camera'),
            ),
            ListTile(
              leading: const Icon(Icons.photo_library),
              title: Text('task_detail.evidence.choose_from_gallery'.tr()),
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              tooltip: 'task_detail.refresh_tooltip'.tr(),
              onPressed: _load,
        ...
        floatingActionButton: _canAddOrDeleteEvidence
            ? PopupMenuButton<String>(
                tooltip: 'task_detail.evidence.add_tooltip'.tr(),
                onSelected: (v) {
                ...
                children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Wrap(
                            ...
                            runSpacing: 6,
                            children: [
                              _kv('task_detail.fields.status'.tr(), t.status),
                              if (t.size != null) _kv('task_detail.fields.size'.tr(), t.size!.displayName),
                              if (t.assigneeName != null) _kv('task_detail.fields.assignee'.tr(), t.assigneeName!),
                              if (t.deadline != null) _kv('task_detail.fields.deadline'.tr(), DateFormat('dd/MM/yyyy').format(t.deadline!)),
                              if (t.githubBranch != null && t.githubBranch!.isNotEmpty) _kv('task_detail.fields.branch'.tr(), t.githubBranch!),
                              if (t.pullRequestUrl != null && t.pullRequestUrl!.isNotEmpty) _kv('task_detail.fields.pr'.tr(), t.pullRequestUrl!),
                              if (t.merged == true) Chip(label: Text('task_detail.fields.merged'.tr())),
                            ],
                          ),
                        ],
                      ),
                    ...
                    children: [
                      ...
                      if (evidences.isNotEmpty && _canAddOrDeleteEvidence)
                        TextButton.icon(
                          ...
                          label: Text('task_detail.evidence.delete_all'.tr()),
                        ),
                    ],
                  ),
                  ...
                  GridView.builder(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      final raw = (ev['url'] ?? ev['path'] ?? ev['fileUrl'] ?? '') as String;
                        ...
                        children: [
                          Positioned.fill(
                            child: InkWell(
                              ...
                              child: ClipRRect(
                                borderRadius: BorderRadius.circular(8),
                                child: _NetImage(url, fit: BoxFit.cover),
                              ),
                            ),
                          ),
...
/// Ảnh mạng kèm Authorization header (nếu có) + tuỳ chọn cache-bust khi mở fullscreen
