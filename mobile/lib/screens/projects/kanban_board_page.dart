...
class _KanbanBoardPageState extends State<KanbanBoardPage> {
  ...
  void initState() {
    ...
    try {
      ...
      // ⚠️ nếu hệ thống không có id, bạn có thể thay bằng username
      userId = acc?.id;
    } catch (_) {
      role = '';
      username = null;
  ...
  bool _canOpenTask(ProjectTaskModel task) {
    ...
    // ✅ Khi phase COMPLETED: chỉ ADMIN / MANAGER / PM của CHÍNH dự án
    if (_phaseLocked) {
      return _isAdmin() || _isManager() || _isProjectPM();
    }

    // Bình thường (phase chưa completed)
    if (_isViewerOnlyRole()) {
      return s != 'COMPLETED';
    }
    return true; // ADMIN/MANAGER/PM (không locked) mở được
  }

  bool _canStartDrag(ProjectTaskModel task) {
    if (_phaseLocked) return false; // ✅ khóa toàn bộ drag khi phase COMPLETED
  ...
  bool _isTransitionAllowed(String from, String to) {
    if (_phaseLocked) return false; // ✅ khóa toàn bộ transition khi phase COMPLETED
  ...
  Future<bool> _ensureInReviewPrereqs(ProjectTaskModel task) async {
    ...
    final ok = await showModalBottomSheet<bool>(
      ...
      builder: (_) => _InReviewSheet(
        task: task,
        lockEvidence: _phaseLocked, // ✅ phase COMPLETED ⇒ khóa upload/xóa
      ),
    );
  ...
  Widget build(BuildContext context) {
    // ƯU TIÊN PHASE NAME. Nếu không có thì dùng "Kanban".
    final String title =
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Stack(
                          children: [
                                  ...
                                  Expanded(
                                    child: DragTarget<ProjectTaskModel>(
                                      onWillAccept: (incoming) {
                                        if (incoming == null || _phaseLocked) return false; // ✅ khóa drop
                                      ...
                                      onAcceptWithDetails: (details) {
                                        if (_phaseLocked) return; // ✅ an toàn
                                        ...
                                        return AnimatedContainer(
                                          ...
                                          child: ListView.builder(
                                            ...
                                            itemBuilder: (context, taskIndex) {
                                                        ...
                                                        final changed = await Navigator.of(context).push(
                                                          MaterialPageRoute(
                                                            builder: (_) => TaskDetailPage(
                                                              task: task,
                                                              lockEvidence: _phaseLocked, // ⬅️ phase COMPLETED ⇒ khóa upload/xóa
                                                            ),
                                                          ),
                                                        );
                                                        if (changed == true) {
                                              ...
                                              // Cho phép nhận drop vào vị trí của card này
                                              ...
                                              // Nếu không được drag thì chỉ trả về droppableCard (vẫn nhận drop nếu chưa locked)
                            ...
                            // Edge drag areas (kéo sang trang trái/phải) — sẽ không kích nếu không drag
                            if (index > 0)
                              Positioned(
                                left: 0,
                                top: 0,
...
class _InReviewSheetState extends State<_InReviewSheet> {
  ...
  Future<void> _pickAndUpload() async {
    if (widget.lockEvidence) return; // ✅ chặn upload khi phase COMPLETED
  ...
  Future<void> _deleteEvidence(int evidenceId) async {
    if (widget.lockEvidence) return; // ✅ chặn xóa khi phase COMPLETED
  ...
  Future<void> _clearAll() async {
    if (widget.lockEvidence) return; // ✅ chặn xóa tất cả khi phase COMPLETED
            ...
            children: [
              Row(
                children: [
                  ...
                  // ✅ Ẩn nút "Xóa tất cả" khi lockEvidence
              ...
              Row(
                children: [
                  // ✅ Ẩn/disable Upload khi lockEvidence
                ...
                GridView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return _EvidenceThumb(
                      ...
                      // ✅ Ẩn nút xóa từng ảnh khi lockEvidence
